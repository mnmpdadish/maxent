...
//	cout<<"fname_out: "<<fname_out<<endl;
	...
	{
		cout<<"utilities::copy_file(): file "<<fname_in<<" or file "<<fname_out<<" does not exist.\n";
		return false;
	}
...
dcomplex utilities::cx_quadInteg1D(cx_fctPtr1 func, double lims[2], double tol, int nbEval[], void *params[])
{
...
dcomplex utilities::cx_quadStep1D(cx_fctPtr1 func,  double lims[2], dcomplex vals[3], double tol, int nbEval[], double hmin, void *params[])
{
...
void utilities::spline_coeffs_LC(vec x0, vec F, vec LC, vec coeffs_LC, vec &coeffs)
{
	int N0=x0.n_rows;
...
void utilities::spline_coeffs_rel(double *x0, double *V, int N0, double *coeffs)
{
	int j;
...
//! calcule les coefficients du spline cubique pour V(x), la taille de coeffs doit etre 4*(N0-1), en entree,
//coeffs[0] et coeffs[1] contiennent les derivees de V(x) a x_1 et x_N0, la taille de x0 et V est N0 et celle de coeffs est 4*(N0-1) 
void utilities::spline_coeffs(double *x0, double *V, int N0, double *coeffs)
{
	int j;
...
double utilities::spline(double x, double *x0, double *V, int N0, double *coeffs)
{
	int ind;
	double a, b, c, d;
...
void utilities::spline_coefficients(double *vec_coeff,const double *xx,const double *yy,const double *fp,const int size)
{
...
double utilities::Interp_1d_spline(const double x,const double *coeff,const double *xx,const int size)
{
...
int utilities::locate(const double x,const double *xx,const int size)
{
...
double utilities::oned_spline_integral(const double *coeff,const double *xx,const double *yy,const int size)
{
	double sum = 0.0;
	int n = size-1;
	for (int r = 0;r<n;r++)
...
bool utilities::find_zero(fctPtr func, double init[], double params[], double root[])
{
...
//zero finding routine
bool utilities::find_zero(fctPtr func, double init[], double params[], double root[], double lims[])
{
	double tol=1.0e-15, tol2=1.0e-10, eps=2.0*EPSILON;
	int nbItermax=15;
	int nbIter=0;
	int nbEvaln=0;
	int ind1, ind2, ind3, indm, indp;
...
// find the minimum (or the root) of a function
bool utilities::find_min(fctPtr func, double init[], double params[], double xmin[])
{
	double tol=1.0e-12;
	int nbItermax=50;
	int nbIter=0;
...
double utilities::quadInteg2D(fctPtr func,	double limx[2], double limy[2], double tol, int nbEval[], double params[])
{
	nbEval[0]=0;
	
	IntPtr inInteg=&utilities::innerInteg;
	
	return quadInteg(func, limy, tol, nbEval, params, inInteg, limx);
}


double utilities::innerInteg(double y, double tol, int nbEval[], double params[], fctPtr func, double limx[2])
{
	params[0]=y;
...
double utilities::quadInteg(fctPtr func, double lims[2], double tol, int nbEval[], double params[], IntPtr inInteg, double limx[2])
{
	double a=lims[0], b=lims[1];
...
double utilities::quadInteg1D(fctPtr1 func, double lims[2], double tol, int nbEval[], void *params[])
{
	double a=lims[0], b=lims[1];
...
double utilities::quadStep1D(fctPtr1 func,  double lims[2], double vals[3], double tol, int nbEval[], double hmin, void *params[])
{
	int nbEvalmax=100000000;
	double a=lims[0], b=lims[1];
...
double utilities::quadStep(fctPtr func,  double lims[2], double vals[3], double tol, int nbEval[], double hmin, double params[],
...
inline void utilities::pade_cont_frac_coef(dcomplex *func, dcomplex *x, int N, dcomplex *coef)
{
	int l,m;
...
dcomplex utilities::pade(dcomplex x, int n, dcomplex *x0, dcomplex *coef)
{
	dcomplex A, B;
	dcomplex q;
...
dcomplex utilities::pade_recursion(int indFunc, int indx, dcomplex *func, dcomplex *x, dcomplex *coef)
{
	if (indFunc>0)
	{
...
void utilities::find_roots_2pol(dcomplex coef[], dcomplex roots[])
{
	double eps=1.0e-15;
...
void utilities::find_roots_3pol(dcomplex coef[], dcomplex roots[])
{
	dcomplex I(0,1);
...
void utilities::find_roots_4pol(dcomplex coef[], dcomplex roots[])
{
